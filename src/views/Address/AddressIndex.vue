...
export default {
  ...
  methods: {
    ...
    editAddress(addressId){
      if (this.$route.query.type==='select'){ //检测是否是从订单页跳转，如果是则为选择
        let orderAddress=this.addressList.filter(v=>{
          return v.id===addressId
        })
        localStorage.setItem('orderAddress',JSON.stringify(orderAddress)) //将选择的地址存入本地存储
        this.$router.back()
      }
      this.$router.push({
        path:'/addOrEdit',
        query:{
          id:addressId //如果是编辑，则需要传递地址id过去
        }
      })
    }
  },
