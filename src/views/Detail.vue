...
export default {
  ...
  methods: {
    ...
    load() {
      if (this.scroll){ //避免重新刷新当前网页出现scroll为空从而导致refresh等一系列方法失效错误
        this.scroll.refresh()
      }else {
        this.getScroll();
        this.scroll.refresh();
    ...
    getScroll(){
      ...
      this.scroll.on('scroll', (pos) => {
        ...
        if (scrollY <= -100) { //取负数是因为better-scroll回弹效果，可以往上滚一段距离，往上滚为正数，若取绝对值，往上滚会造成secondHeader出现
          this.isShow = false;
        } else {
          this.isShow = true;
        }
  ...
  activated() {
    ...
    if (product !== this.product.title) { //若不是相同商品，则发送请求
      this.getData();
    }
  }
}
