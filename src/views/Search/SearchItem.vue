...
export default {
  ...
  computed: {
    orderBy() {
      let obj = this.orderList.data[this.orderList.currentIndex];//当前点击的对象
      let val = obj.status === 1 ? 'ASC' : 'DESC';
      return {
        [obj.key]: val //obj.key为price或者num
      }
    }
  },
  methods: {
    async searchData() {
      if (this.orderList.currentIndex){ //价格和销量
        await http.$axios({
          url: '/api/search/results',
          params: {
            title: this.$route.query.key,
            ...this.orderBy //对象转换为price:DESC格式
          }
        }).then(v => {
          this.searchList = v
        })
        ...
        await http.$axios({
          url: '/api/search/results',
          params: {
            title: this.$route.query.key, //获取搜索的值
          }
        }).then(v => {
          this.searchList = v
        })
